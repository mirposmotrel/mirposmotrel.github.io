...
$(document).ready(function() {
	...
	var slider_settings = {
		...
		prevArrow: '<div class="slick-prev slick-arrow">' +
			'<svg class="icon prev" width="30px" height="30px"><use xlink:href="#prev"></use></svg>' +
			'</div>',
		nextArrow: '<div class="slick-next slick-arrow">' +
			'<svg class="icon next" width="30px" height="30px"><use xlink:href="#next"></use></svg>' +
			'</div>'
	}
	slider = $('.mob-slide');
	// slick disable
  ...
  // submit формы и отправлка facebook Lead после проверки валидации 
  
  $('#form').validator().on('submit', function (e) {
    if (e.isDefaultPrevented()) {
      // handle the invalid form...
    } else {
      $(".form-btn").html('Отправляю...').attr('disabled',true);
      fbq('track', 'Lead');
    }
  })
